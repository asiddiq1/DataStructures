...
int HashSlash(const std::string& key)
{
	int hash = 0;
...
typedef ics::HashSet<std::string, HashSlash> FollowSet;
...
int WordQ(const WordQueue& w)
{
	return w.size();
}
typedef ics::HashMap<WordQueue,FollowSet, WordQ>   Corpus;
...
//  read_corpus time                   = 356.197
//  print_corpus (sort not print) time = 0.108209

//Only ArrayQueue Implementations (HeapPriorityQueue, HashMap, and HashSet)
//  read_corpus time                   = 361.221
//  print_corpus (sort not print) time = 0.213121

//Briefly explain (in a comment below) what is interesting
//about the times above (and why it occurs).
//The time with the HashSet is slightly slower than the one without HashSet implementation.
//Since the difference between the 2 is based on inclusion of the HashSet,
//it is probable that the one with the HashSet is slower since it added an implementation that goes through both
//the array and linked nodes.
